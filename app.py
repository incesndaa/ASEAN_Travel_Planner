...
if select == 'Home':
    ...
    if st.button("Cek Jarak"):
        if asal == tujuan:
            st.error("Negara tujuan dan negara asal tidak boleh sama!")
        else:
            if asal and tujuan:
    ...
    if st.button("Hitung Anggaran"):
        if biayaTransportasi <= 0 or biayaAkomodasi <= 0 or biayaMakanan <= 0 or biayaTakTerduga <= 0:
            st.error("Data tidak boleh kosong")
        else:
    ...
    if st.button("Hitung Simulasi Tabungan"):
        if anggaranPerjalanan <= 0 or durasiTabungan <= 0 or jumlahYangSudahDitabung <= 0:
            st.error("Data tidak boleh kosong")
        else:
